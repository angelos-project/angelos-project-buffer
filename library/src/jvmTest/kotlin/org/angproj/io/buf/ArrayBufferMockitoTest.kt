...
class ArrayBufferMockitoTest {

    @Mock
    private lateinit var buffer: ArrayBuffer<Int>
    ...
    @Test
    fun testLastIndex() {
        whenever(buffer.lastIndex).doReturn(1023)
        assertEquals(1023, buffer.lastIndex)
    }

    @Test
    fun testIndices() {
        whenever(buffer.indices).doReturn(0..1024)
        assertEquals(0..1024, buffer.indices)
    }

    @Test
    fun testSet() {
        doNothing().whenever(buffer).set(0, 100)
        assertEquals(Unit, buffer.set(0, 100))
    }

    @Test
    fun testGet() {
        whenever(buffer.get(0)).doReturn(100)
        assertEquals(100, buffer.get(0))
    }
