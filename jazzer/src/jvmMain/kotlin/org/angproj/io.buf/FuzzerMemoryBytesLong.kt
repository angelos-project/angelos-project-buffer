...
public object FuzzerMemoryBytesLongKt : FuzzPrefs() {
    ...
    public fun fuzzerTestOneInput(data: FuzzedDataProvider) {
        ...
        repeat(loops) {
            ...
            bytes.setLong(it * typeSize, value)
        }

        repeat(loops) {
            val expected = memory.getLong(it * typeSize)
            val actual = bytes.getLong(it * typeSize)
