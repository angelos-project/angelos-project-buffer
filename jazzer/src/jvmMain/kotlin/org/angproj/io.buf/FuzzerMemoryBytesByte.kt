...
public object FuzzerMemoryBytesByteKt : FuzzPrefs() {
    ...
    public fun fuzzerTestOneInput(data: FuzzedDataProvider) {
        ...
        repeat(loops) {
            ...
            bytes.setByte(it * typeSize, value)
        }

        repeat(loops) {
            val expected = memory.getByte(it * typeSize)
            val actual = bytes.getByte(it * typeSize)
