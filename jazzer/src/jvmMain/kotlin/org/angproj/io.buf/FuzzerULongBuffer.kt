...
public object FuzzerULongBufferKt : FuzzPrefs() {

    @JvmStatic
    public fun fuzzerTestOneInput(data: FuzzedDataProvider): Unit = withUtility {
        BufMgr.withRam(DataSize._1K) {
            val buffer = it.asULongBuffer()
            val array = LongArray(buffer.size) { data.consumeLong() }

            array.forEachIndexed { index, value ->
                buffer[index] = value.conv2uL()
            }
            array.forEachIndexed { index, value ->
                assertEquals(value, buffer[index].conv2L())
            }
        }
    }
