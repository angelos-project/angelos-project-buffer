...
public object FuzzerMemoryBytesShortKt : FuzzPrefs() {
    ...
    public fun fuzzerTestOneInput(data: FuzzedDataProvider) {
        ...
        repeat(loops) {
            ...
            bytes.setShort(it * typeSize, value)
        }

        repeat(loops) {
            val expected = memory.getShort(it * typeSize)
            val actual = bytes.getShort(it * typeSize)
